...
// Noise words
const NOISE = [
  'cariin','cari','search','browsing','dong','deh','bro','bang','gan',
  'gue','gw','aku','saya','tolong','please','lu','lo','kamu','anda',
  'coba','minta','tau','tahu','yang','itu','ini','ada','apa',
  'gimana','gmn','trus','terus','sebenernya','beneran','emangnya',
  'nih','sih','lah','kah','nah','jadi','kan','udah','sudah','belum',
  'masih','emang','sekarang','tadi','tolong','mohon','berita','info',
  'informasi','soal','tentang',
];
...
const DOMAIN_MAP = [
  ...
  { re: /gerbong.*perempuan/i, kw: 'gerbong perempuan kereta kebijakan menteri' },
  { re: /lagu.*67|67.*lagu/i,  kw: 'lagu 67 viral tiktok arti lirik' },
  { re: /ai.?agent|framework/i,kw: 'ai agent framework android termux ringan' },
];
...
function extractTopicFromHistory(history) {
  ...
  for (const { re, kw } of DOMAIN_MAP) {
    if (re.test(text)) { found.push(kw); if (found.length >= 2) break; }
  }
  return found.join(' ');
}
...
function generateQuery(input, history = '') {
  ...
  const isFollowUp = input.split(/\s+/).length < 8 &&
    /\b(itu|nya|tadi|gimana|gmn|trus|terus|lanjut|hasilnya|responnya|kebijakannya|mereka|dia)\b/i.test(input);
...
async function smartSearch(input, history = '') {
  ...
  for (const r of results.slice(0, 3)) {
    ...
    try {
      ...
      if (page.content.length > 150) {
        pages.push({ title: r.title, url: r.url, content: page.content.slice(0, 2500) });
      }
    } catch {}
  }
  ...
  return {
    needed: true, query,
    data: { snippets, pages, hasPages: pages.length > 0, count: results.length },
  };
}
