...
function parseJSON(raw) {
  ...
  // Coba bersihkan trailing koma atau tanda kutip aneh
  try {
    const cleaned = raw
      .replace(/,\s*}/g, '}')
      .replace(/,\s*]/g, ']')
...
- Pertanyaan tentang error SISTEM/AI (bukan kode user) → chat
  Contoh: 'eror kenapa tadi?', 'kenapa gagal?', 'itu error apa?' → chat
...
{"intent":"...","should_browse":false,"confidence":0.0,"reason":"..."}`;
  ...
  try {
    const raw      = await provider.call(prompt, { maxTokens: 100, temperature: 0.1 });
    const parsed   = parseJSON(raw);
    if (!parsed) return FALLBACK(classifierIntent);

    const validIntents = ['chat','research','build','fix','automation'];
    if (!validIntents.includes(parsed.intent)) return FALLBACK(classifierIntent);
