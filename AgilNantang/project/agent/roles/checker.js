...
SUMMARY: [1 kalimat]`;
  ...
  let output;
  try {
    output = await provider.call(prompt, { maxTokens: 300 });
  } catch {
    // Timeout → auto PASS
    return { ok: true, output: 'STATUS: PASS\nISSUES: none\nSUMMARY: Auto-passed (timeout).', role: 'checker' };
  }
  ...
  if (isFail) {
    return { ok: false, output, error: 'Checker: ada bug CRITICAL', role: 'checker' };
  }
